...
load("../../EmpiricalStudy/SAT_Analysis/Preprocessed_SATdata.RDATA")
load("../../EmpiricalStudy/SAT_Analysis/collected_Data_Fits_Predicts.RData")
...
Sim_RTQuants_corr_rating  <- data.frame()
Sim_MRating_corr_cond <- data.frame()
SubjTiming_pars <- matrix(c( 1, 1,2, 2, 5, 5, 0.5, 3, 0.5, 3, 0.5, 3), ncol=2)

if (!file.exists("simulation_subjtiming_multistep_SAT.RData")) {
  #nrow_parts <- table(Data$participant)
  set.seed(2904)
  for (K in 1:nrow(SubjTiming_pars)) {
    #simulations_subjtiming <- data.frame()
    
    dynaViTE_fits$Timerate <- SubjTiming_pars[K, 1]
    dynaViTE_fits$Timediff <- SubjTiming_pars[K, 2]
    ...
    sim_combine <- function(paramDf) {
      ...
      simu <- simulatedynaViTE_SAT_subjtimig(paramDf, 1e+4,
                                             df, model=paramDf$model, maxrt = 30)
      return(simu)
      #return(cbind(model=paramDf$model, participant=paramDf$participant, simu))
    }

    simulations_subjtiming <- dynaViTE_fits %>% group_by(participant) %>%
      do(sim_combine(.))
    ...
    recompute_rating <- function(conf, cur_participant) {
      ...
      thresholds <- c(quantile(conf, probs = cumsum(proportions))[1:5], Inf)
    ...
    Sim_MRating_corr_cond1 <- simulations_subjtiming %>% 
      group_by(participant, condition, correct, SAT) %>%
      summarise(MRating = mean(rating), .groups = "drop") 
    Sim_MRating_corr_cond_SE <- Sim_MRating_corr_cond1 %>%
      reframe(summarySEwithin(pick(everything()),measurevar = "MRating", 
                              idvar = "participant", withinvars = c("condition", "correct", "SAT"))) %>%
      select(condition, correct, SAT, sewithin = se) 
    Sim_MRating_corr_cond <- Sim_MRating_corr_cond1 %>%
      group_by(condition, correct, SAT) %>%
      summarise(MRating=mean(MRating, na.rm=TRUE), .groups="drop") %>%
      mutate(correct=as.factor(correct), SAT = as.factor(SAT),condition=as.factor(condition)) %>%
      left_join(Sim_MRating_corr_cond_SE, by=c("condition", "correct", "SAT"))%>%
      mutate(Timediff = dynaViTE_fits$Timediff[1],
             Timerate = dynaViTE_fits$Timerate[1],
             condition = factor(condition, labels = levels(Preds_MRating_corr_cond$condition))) %>%
      rbind(Sim_MRating_corr_cond)
...
#                                                  confidence ="Confidence measure"))
Sim_MRating_corr_cond_plot <- Sim_MRating_corr_cond%>%
  mutate(Timerate =paste("italic(d[T])==", Timerate),
         Timediff = paste("italic(s[T])==", Timediff))
Sim_MRating_corr_cond_plot$Time_pars <- paste("d[T]==", Sim_MRating_corr_cond$Timerate, 
                                         ", s[T]==", Sim_MRating_corr_cond$Timediff, sep="")

p_MRating <- ggplot(Sim_MRating_corr_cond_plot,
  ...
  facet_nested(rows=c(vars(Timerate), vars(Timediff)), cols=c(vars(SAT)),
               labeller = labeller(SAT=label_value,
                                   Timerate=label_parsed,
                                   Timediff=label_parsed))+ #,dir="v"
  scale_x_discrete(name="Longer line length [mm]")+
  scale_color_manual(values= two_colors_correct, breaks=c(1,0),
                     name = "Predicted by\ndynaViTE", labels=c("Correct", "Wrong")) +
  # scale_fill_manual(values= two_colors_correct, breaks=c(1,0),
  #                   name = "Predicted by\ndynaViTE", labels=c("Correct", "Wrong")) +
  scale_shape_manual(values=c(21,17),breaks=c(1,0),
                     name = "Simulated with \nsubjective time signal", labels=c("Correct", "Wrong"))  +
...
ggsave("../../../Supplement/figures/simul_subjtiming_SAT_meanconf.eps",
      width = 17.62, height=17, units="cm",dpi=600, device = cairo_ps)
ggsave("figures/simul_subjtiming_SAT_meanconf.tiff",
...
ggplot()+
  ...
  facet_nested(rows=c(vars(Timerate), vars(Timediff)), cols=c(vars(SAT), vars(correct)),
               labeller = labeller(correct=label_value, 
                                   SAT=label_value,
                                   Timerate=label_parsed,
                                   Timediff=label_parsed))+ #,dir="v"
  theme_bw() +
  theme(plot.margin = margin(0, 0, 0, 0, "cm"),
        ...
        panel.spacing=unit(0, "lines"))
 # # Only for manuscript generation
ggsave("../../../Supplement/figures/simul_subjtiming_SAT_RTQuantsConf.eps",
       width = 15, height=16, units="cm",dpi=600, device = cairo_ps)
ggsave("figures/simul_subjtiming_SAT_RTQuantsConf.tiff",
       width = 15, height=16, units="cm",dpi=600)
# 
# ## Suppl Fig 6-8: Fitted response distributions            ----
# model_colors = c("#D81B60", "#1E88E5", "#FFC107","#004D40")
# for (i in 1:3) {
#   cur_experiment <- sort(unique(fits$experiment))[i]
#   p_ratingdist_KonfMask <- ggplot(data=filter(Sim_RatingDist_corr_cond==cur_experiment), aes(x=rating, y=p))+
#     geom_bar(stat = "identity", show.legend = FALSE, fill="white", col="black")+
#     geom_point(data=filter(Preds_RatingDist_corr_cond==cur_experiment),
...
#   temp <- temp %>% group_by(participant, rating) %>%
