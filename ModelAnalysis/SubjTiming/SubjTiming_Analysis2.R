...
if (!file.exists("simulation_subjtiming_multistep.RData")) {
  ...
  for (K in 1:nrow(SubjTiming_pars)) {
    ...
    simulations_subjtiming <- dynaViTE_fits %>% group_by(participant, experiment) %>%
      do(simulatedynaViTE_subjtimig(select(.,where(~!all(is.na(.x)))), n=1e+4, simult_conf=TRUE))
...
p_MRating <- ggplot(Sim_MRating_corr_cond_plot,
  ...
  facet_nested(rows=c(vars(Timerate), vars(Timediff)), cols=c(vars(experiment)),
               scales = "free_x", drop=TRUE,
               labeller = labeller(experiment=label_value,
...
Sim_RTQuants_corr_rating_plot <- Sim_RTQuants_corr_rating %>%
  ...
  mutate(Time_pars =paste("dT=", Timerate, "\nsT=", Timediff, sep=""))%>%
  mutate(Timerate =paste("italic(d[T])==", Timerate),
         Timediff = paste("italic(s[T])==", Timediff))
...
ggplot()+
  ...
  #facet_nested(rows=vars(Time_pars), cols=c(vars(experiment), vars(correct)))+ #,dir="v"
  facet_nested(rows=c(vars(Timerate), vars(Timediff)), cols=c(vars(experiment), vars(correct)),
               labeller = labeller(correct=label_value, 
                                   experiment=label_value,
                                   Timerate=label_parsed,
                                   Timediff=label_parsed))+ #,dir="v"
  theme_bw() +
  theme(plot.margin = margin(0, 0, 0, 0, "cm"),
