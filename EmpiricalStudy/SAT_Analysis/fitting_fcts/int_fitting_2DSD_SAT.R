fitting2DSDSAT <- function(df, nConds, nRatings, nSATs, fixed, sym_thetas,
                        optim_method, opts,
                        logging, filename,
  ...
  # init_grid <- init_grid[sample(1:nrow(init_grid), 100),]
  Rcpp::sourceCpp("fitting_fcts/RNG_WEV_SAT3.cpp")
  init_thetas <- get_thetas_for_init_grid_dynWEV_simulations(init_grid, df, nRatings, 
  ...
  parnames <- c('sz', paste("v", 1:nConds, sep=""),paste("a", 1:nSATs, sep=""),
                'st0', 'sv', 't0', cols_theta,'tau0','lambda')
  ...
  if (useparallel) {
    logL <-
      parApply(cl, inits, MARGIN=1,
               function(p) try(neglikelihood_2DSDSAT_bounded(p, df,  nConds, nRatings, nSATs, fixed, mint0, maxtau0, sym_thetas, precision),
                               silent=TRUE))
    #stopCluster(cl)
  } else {
    logL <-
  ...
  if (logging==TRUE) {
    logger::log_success(paste("Initial grid search took...",as.character(round(as.double(difftime(Sys.time(),t00,units = "mins")), 2))," mins"))
                    ...
                    # sz,  v1, v2,....,, a1, a2,...,     st0, sv, t0,thetaLower1, dthetaLower2.., thetaUpper1... (or theta1,...),  tau0, lambda
  lower_optbound <- c(0,  rep(0, nConds),rep(0.01, nSATs), 0,   0, 0, rep(c(-Inf,  rep(0, nRatings-2)), 2-as.numeric(sym_thetas)), 0,   0)[!(parnames %in% names(fixed))]
  upper_optbound <- c(1,  rep(Inf,nConds),rep(Inf, nSATs),Inf, Inf,1, rep(Inf, (2-as.numeric(sym_thetas))*(nRatings-1)),           1,  Inf)[!(parnames %in% names(fixed))]
    ...
    for (i in 1:opts$nAttempts){
      ...
      for (l in 1:opts$nRestarts){
        ...
        try(m <- bobyqa(par = start,
                        fn = neglikelihood_2DSDSAT_bounded,
  ...
  if(exists("fit") && is.list(fit)){
    ...
    if (sym_thetas) {
      parnames <- c(paste("v", 1:nConds, sep=""), paste("a", 1:nSATs, sep=""), 'sv', 'z', 'sz', 't0','st0', paste("theta", 1:(nRatings-1), sep=""), 'tau0',  'lambda')
    } else {
      parnames <- c(paste("v", 1:nConds, sep=""), paste("a", 1:nSATs, sep=""), 'sv', 'z', 'sz', 't0','st0', paste("thetaLower", 1:(nRatings-1), sep=""), paste("thetaUpper", 1:(nRatings-1), sep=""), 'tau0', 'lambda')
    }
    res <- res[, parnames]
...
neglikelihood_2DSDSAT_bounded <-   function(p, data,
                                            nConds, nRatings, nSATs, fixed, mint0, maxtau0,  sym_thetas=FALSE, precision=1e-5)
{
  ...
  negloglik <- -LogLikWEVSAT(data, paramDf, "2DSDT",    precision, stop_on_error=FALSE)
  return(negloglik)
}
