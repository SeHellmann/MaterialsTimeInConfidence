...
#' @rdname LogLikWEV
#' @export
LogLikWEVSAT <- function(data, paramDf, model="dynaViTE",precision=1e-5, stop_on_error = TRUE, data_names = list(), ...) {
  ...
  names_missing <- !(c("SAT", "condition","response","stimulus","rating", "rt", "sbj", "correct", "RT2") %in% names(data_names))
  data_names <- c(data_names,
                  setNames(as.list(c("SAT", "condition","response","stimulus","rating", "rt", "sbj", "correct", "RT2")[names_missing]),
                           c("SAT", "condition","response","stimulus","rating", "rt", "sbj", "correct", "RT2")[names_missing]))
  if (is.null(data[[data_names$condition]])) data[[data_names$condition]] <- 1
  if (is.null(data[[data_names$SAT]])) data[[data_names$SAT]] <- 1
  
  data <- data %>% mutate(response = ifelse(response==-1, "lower", "upper"), #response = if_else(.data[[data_names$response]]==sort(unique(data[[data_names$response]]))[1],"lower","upper"),
                          ...
                          SAT = as.numeric(factor(.data[[data_names$SAT]],levels = sort(unique(data[[data_names$SAT]])))),
                          Tau = .data[[data_names$RT2]] - paramDf$tau0 + paramDf$t0)
  ...
  probs <- with(data, switch(which(model== c("dynaViTE", "2DSD")),
                             dWEV(rt, vth1,vth2,
                                        ...
                                        tau=Tau, a=A,
                                        v = M_drift,
                                        t0 = paramDf$t0, z = .5, sz = paramDf$sz, st0=paramDf$st0,
