...
#' @export
fitRTConfModelsSAT <- function(data, models = c("dynaViTE", "2DSD", "PCRMt"),
                      nRatings = NULL, fixed = list(sym_thetas = FALSE),
  ...
  if (any(!grepl("dynaViTE|dynWEV|2DSD", models))) {
    stop("all models must be one of:
    'dynaViTE', 'dynWEV', '2DSDT', or '2DSD'")
  }
  ...
  if (sym_thetas) {
    outnames <- c("model", "sbj","negLogLik", "N", "k", "BIC", "AICc", "AIC", "fixed",
                  ...
                  "sz", "sv", "tau0", "w", "svis", "sigvis", "lambda")
  ...
  call_fitfct <- function(X) {
    ...
    res <- fitRTConfSAT(data_part, model = cur_model,
              nRatings = nRatings, fixed = fixed,
              precision=precision,
  ...
  if (parallel.models) {
    ...
    clusterExport(clmodels, c("data", "fixed", "nRatings",  "models",
                              "logging", "opts", "optim_method",
                              "parallel.single", "n.cores.single", "precision",
                              "outnames", "call_fitfct"), envir = environment())
    clusterExport(clmodels, c("fitRTConfSAT"), envir = environment())
